...
export class ReservationsController {
  ...
  async bookSlot(
    ...
    const reservation =
      await this.reservationsService.bookSlot(createReservationDto);
  ...
  @Get(':providerId')
  async findUpcomingByProvider(
    @Param('providerId') providerId: string,
  ): Promise<ReservationResponseDto[]> {
    const reservations =
      await this.reservationsService.findUpcomingByProvider(providerId);
